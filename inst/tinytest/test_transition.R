...
# Per-user defaults are registered with the LTX-2.3 fallbacks.
expect_equal(getOption("xtx.transition.num_frames"), 121L)
expect_equal(getOption("xtx.transition.conditioning_frames"), 17L)

# Point at a dummy base; validation fails before any request is made, so the
# server is never contacted in these cases.
options(xtx.wan2gp_api_base = "http://127.0.0.1:59999")

# Missing start_clip.
expect_error(transition("/no/such/clip.mp4"), "start_clip file not found")

# A real start_clip so validation gets past the file check.
...
# conditioning_frames must be in 1..num_frames-1.
expect_error(
  transition(clip, num_frames = 49L, conditioning_frames = 49L),
  "conditioning_frames"
)
...
# Generates a real (minimal) clip, so it needs the WanGP API up.
