...
.get_api_base <- function() {
    ...
    if (is.null(base) || nchar(base) == 0) {
        stop("API base URL not set. Use set_api_base() to configure it.\n",
             "Example: set_api_base(\"https://api.openai.com\")",
             call. = FALSE)
    }
    base
}
...
#' @keywords internal
.get_api_key <- function(required = TRUE) {
    key <- getOption("xtx.api_key")
    if (required && (is.null(key) || nchar(key) == 0)) {
        stop("API key not set. Use set_api_key() to configure it.\n",
             "Example: set_api_key(\"sk-...\")", call. = FALSE)
    }
    key
}
...
.get <- function(endpoint) {
    .request(endpoint, method = "GET", expect_binary = FALSE)
}

#' POST multipart form data
#' @keywords internal
.post_multipart <- function(endpoint, form_data, expect_binary = FALSE) {
    base <- .get_api_base()
    url <- paste0(base, endpoint)
...
#' @keywords internal
.save_image <- function(image_data, file, response_format) {
    if (response_format == "url" && !is.null(image_data$url)) {
        # Download from URL
        tryCatch({
            h <- curl::new_handle()
            curl::handle_setopt(h, timeout = .get_timeout())
