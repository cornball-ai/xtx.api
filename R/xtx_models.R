...
xtx_models <- function() {
    ...
    if (!is.null(result$data)) {
        ...
        if (is.data.frame(models) && "id" %in% names(models)) {
            image_models <- models[grepl("dall-e|image|diffusion", models$id,
                    ignore.case = TRUE),]
...
xtx_supported_models <- function(backend = NULL) {
    models <- data.frame(
                         ...
                         backend = c("openai", "openai", "diffuser", "diffuser"),
                         type = c("txt2img", "txt2img/img2img/variations", "txt2img/img2img",
                                  "txt2img/img2img"),
                         description = c(
